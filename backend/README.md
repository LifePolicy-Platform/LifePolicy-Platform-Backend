...
複製 `application.yml` 並透過環境變數覆蓋機密值：

```bash
# 必填（正式環境）
export DB_URL=jdbc:mysql://your-host:3306/ins_crm?useSSL=true&serverTimezone=Asia/Taipei
export DB_USERNAME=your_db_user
export DB_PASSWORD=your_db_password
export JWT_SECRET=your-256-bit-random-secret   # 對應 auth.jwt.secret
export JWT_EXPIRATION_SECONDS=28800             # 預設 8 小時
```

> **注意**：`application.yml` 內的 `app.jwt.secret`（舊版 JwtUtil 使用）為**裸字串，無環境變數覆蓋**，必須在正式上線前從程式碼中移除或替換為安全金鑰。
...
> **目前有兩套 JWT 金鑰並存**（新版 `auth.jwt.secret`、舊版 `app.jwt.secret`）。新版 token 的主體為 `AppUserPrincipal`；舊版為純 `String`。呼叫 `/api/v1/auth/me` 時若持有舊版 token 會因 ClassCastException 回傳 500，預計在未來版本移除舊版路徑。
...
以下問題已由 code review 識別，尚待修正：

| 編號 | 嚴重度 | 位置 | 描述 |
|------|--------|------|------|
| #1 | ~~Critical~~ **已修** | `SecurityConfig.java:49` | ~~`anyRequest().permitAll()` 全域開放~~ → 已改為 `.authenticated()` 並補 RBAC 規則 |
| #2 | Critical | `AuthService.java:117` | `updateUser()` 接受任意 roleCode，APPLICANT 可自我升為 ADMIN |
| #3 | Critical | `application.yml:33` | `app.jwt.secret` 為無法覆蓋的裸字串硬編碼；DB 密碼與 JWT secret 的 fallback 值在版本控制中可見 |
| #4 | High | `ClaimAdminController.java:130` | 上傳檔名未清理（Path Traversal）；例外訊息直接回傳洩露伺服器路徑 |
| #5 | High | `AuthController.java:64` | 舊版 JWT 用戶呼叫 `/api/v1/auth/me` 觸發 ClassCastException → 500 |
| #6 | High | `ClaimAdminController.java:57` | `claimNo` 以秒為單位產生，同一秒並發請求必然碰撞 |
| #7 | High | `CallAppointmentCommandServiceImpl.java:163` | `generateRecNo()` 無鎖讀 MAX，並發插入可產生重複 recNo |
| #8 | Medium | `ClaimAuditController.java:122` | `findById(null)` 無 null guard，可能觸發非預期 rollback 導致審核結果遺失 |
| #9 | Medium | `NotificationService.java:29` | `pushToRole()` 缺少 username null 檢查 |
| #10 | Low | `ClaimAdminController.java:90` | Delete 要求 PENDING，但建立時為 SUBMIT，刪除前須先過 audit 步驟，無文件說明 |
